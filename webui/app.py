...
def create_config():
    try:
        ...
        logger.exception("Engine error while creating config")
...
def delete_config(config_id):
    try:
        ...
        logger.exception("Engine error while deleting config %s", config_id)
        return jsonify({'error': 'Internal server error'}), 503
...
def trigger_scrape():
    try:
        ...
        logger.exception("Engine error while triggering scrape")
...
def test_scrape():
    try:
        ...
        logger.exception("Engine error while testing scrape")
        return jsonify({'status': 'error', 'message': 'Internal server error'}), 503
